...
public class ItemizedOverlayWithLabel<Item extends OverlayItem> extends ItemizedIconOverlay<Item> {
	...
	public void drawItem (final Canvas c, final MapView osmv, Item item) {
		...
		osmv.getProjection().toMapPixels(item.mGeoPoint, mFocusedScreenCoords);
		...
		final String itemTitle = (item.mTitle == null) ? UNKNOWN : item.mTitle;
		final String itemDescription = (item.mDescription == null) ? UNKNOWN
				: item.mDescription;
